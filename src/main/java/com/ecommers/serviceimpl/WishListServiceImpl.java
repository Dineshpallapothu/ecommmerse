...
public class WishListServiceImpl implements WishListService {
    ...
    public WishListDto remove(int id) {
        WishList wishList = wishlistRepo.findById(id).orElseThrow(() -> new NotFound("no items available in cart on this id"));
        WishListDto cartDto = new WishListDto(wishList);
    ...
    public WishListDto create(WishListDto wishListDto) {
        Profile profile = profileRepo.findById(wishListDto.getProfileId()).orElseThrow(() -> new NotFound("profile id not found can you check it"));
        Item item = itemRepo.findById(wishListDto.getItemId()).orElseThrow(() -> new NotFound("item id not found can you check it"));
    ...
    public WishListDto update(int id, WishListDto wishListDto) {
        WishList wishList = wishlistRepo.findById(id).orElseThrow(() -> new NotFound("sorry"));
            wishList.getItem().setId(wishListDto.getItemId());
            wishList.setName(wishListDto.getName());
            wishList.getProfile().setId(wishListDto.getProfileId());
            wishlistRepo.save(wishList);
